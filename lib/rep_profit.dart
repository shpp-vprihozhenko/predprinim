...
class RepProfit extends StatefulWidget {
  const RepProfit({Key? key}) : super(key: key);

  @override
  _RepProfitState createState() => _RepProfitState();
}

class _RepProfitState extends State<RepProfit> {
  int curProfit = 0;
  ...
  _formReport(){
    ...
    setState(() {
      sumFrom = curProfit - sPlusFrom + sMinusFrom;
      sumTo = curProfit - sPlusTo + sMinusTo;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Прибыль'),),
      body: Column(
        children: [
          Padding(
            ...
            child: Row(
              ...
              children: [
                Text('Текущая прибыль $curProfit грн.', textScaleFactor: 1.4,),
              ],
            ),
          ),
          Row(
